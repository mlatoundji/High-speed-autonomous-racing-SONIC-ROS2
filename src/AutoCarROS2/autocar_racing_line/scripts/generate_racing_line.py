...
Usage:
    ...
    ros2 run autocar_racing_line generate_racing_line.py --plot
    python3 src/AutoCarROS2/autocar_racing_line/scripts/generate_racing_line.py

Plotting is off by default (avoids matplotlib/NumPy ABI issues on some systems).
Use ``--plot`` only when you need the comparison PNG.
"""
...
def _package_data_dir() -> Path:
    """Resolve .../share/autocar_racing_line/data (install) or source tree data/."""
    try:
        from ament_index_python.packages import get_package_share_directory
        return Path(get_package_share_directory('autocar_racing_line')) / 'data'
    except Exception:
        pass
    ...
    raise FileNotFoundError(
        'Cannot find autocar_racing_line data/. '
        'Run: colcon build --packages-select autocar_racing_line && source install/setup.bash'
    )


def _default_plot_path() -> Path:
    docs = Path(__file__).resolve().parents[4] / 'docs' / 'figures' / 'racing_line.png'
    if docs.parent.is_dir():
        return docs
    return Path.cwd() / 'racing_line.png'


_PKG_DATA = _package_data_dir()
DEFAULT_IN = _PKG_DATA / 'waypoints.csv'
DEFAULT_OUT = _PKG_DATA / 'waypoints_racing.csv'
DEFAULT_PLOT = _default_plot_path()
...
def main():
    ...
    p.add_argument(
        ...
        default=None,
        type=Path,
        help='Write comparison PNG (optional path; default: docs/figures/racing_line.png)',
    )
    args = p.parse_args()
    ...
    if args.plot is not None:
        try:
            ...
            print(
                f'matplotlib unavailable ({type(e).__name__}: {e}); '
                'skipping plot. CSV was written successfully. '
                'For plots on Ubuntu 22.04 try: pip install "numpy<2" matplotlib',
            )
            return
