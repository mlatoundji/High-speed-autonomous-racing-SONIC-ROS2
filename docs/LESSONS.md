...
**Decision**: in-place migration on node startup. If the existing `~/.ros/autocar_lap_times.csv` has the legacy 7-col header, we rewrite the file once with the new header and pad historical rows with empty cells for the 8 new fields. After that, the file is uniform and pandas can read it directly without ragged-row warnings. `docs/baseline_lap_times.csv` is a frozen snapshot and is not touched.
