...
> Target: 10 pages PDF (this Markdown file is converted with `pandoc docs/pp_racing_line_experiment/REPORT.md -o report.pdf`).
...
The aggregate is written to `snapshots/results_<timestamp>.csv` and three plots are saved to `figures/`:
...
The full incident log is kept in [`LESSONS.md`](LESSONS.md); the entries below are the ones that most directly shaped the project's outcome.
...
# bench prints progress and writes snapshots/results_<timestamp>.csv
```
...
The live metrics CSV is at `~/.ros/autocar_lap_times.csv` and frozen snapshots used in this report are in `snapshots/`.
